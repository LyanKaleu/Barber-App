...
const createApi = () => {
    ...
    const setToken = (token: string) => {
        axiosInstance.defaults.headers.common.Authorization = `Beater ${token}`;
    };
    ...
    const getProviders = () => {
        return axiosInstance.get<ApiTypes.ProviderResponse>('providers');
    };
    ...
    const updateProfile = (params: ApiTypes.UpdateProfileParams) => {
        const { name, email, password, password_confirmation, old_password } = params;
